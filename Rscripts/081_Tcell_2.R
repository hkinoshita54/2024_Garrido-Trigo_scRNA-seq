...
# subset
seu <- subset(seu, subset = cellgroup %in% c("Tcell"))    # did not use "TandB" cells in this version
seu <- subset(seu, subset = DERL3 > 0 | MS4A1 >0 | C1QA > 0 | EPCAM >0 | CD79A > 0, invert = TRUE)
seu <- subset(seu, subset = nFeature_RNA > 200 & nFeature_RNA < 8000 & percent.mt < 15)    # mt<15
