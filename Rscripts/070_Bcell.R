...
# subset
seu <- subset(seu, subset = cellgroup %in% c("Bcell", "TandB"))
seu <- subset(seu, subset = CD3D > 0 | CD3E > 0 | CD3G > 0 | C1QA > 0 | EPCAM >0, invert = TRUE)
...
features <- readLines("gene_set/Bcell.txt")
...
FeaturePlot(seu, features = "IGKC", cols = c("lightgrey","darkred"), label = TRUE, repel = TRUE) + NoAxes() + NoLegend()
...
seu <- RenameIdents(
  ...
  `4` = "IgG-P", `8` = "IgG-P",
  `14` = "IgGL-P",
  `0` = "IgA-P", `1` = "IgA-P", `2` = "IgA-P", `10` = "IgA-P", `11` = "IgA-P", `12` = "IgA-P", `16` = "IgA-P", 
  `3` = "IgA-HSP-P",
  `6` = "IgAL-P",
  `7` = "IER-P", 
  `9` = "IGLL5-P", 
...
seu$celltype <- factor(seu$celltype, 
                       levels = c("PB", "IgG-P", "IgGL-P", "IgA-P", "IgA-HSP-P", "IgAL-P", "IER-P", "IGLL5-P", 
                                  "Naive-B", "Bcell", "GC-B", "Mem.-B", "Cycl.-B/P"))
Idents(seu) <- "celltype"
DimPlot(seu, label = TRUE, repel = TRUE) + NoAxes()
...
seu <- readRDS("RDSfiles/seu_070_Bcell_npc30.RDS")
levels <- levels(seu$celltype)
levels[levels %in% c("PB", "IgGL-P")] <- "IgG-P"
levels[levels %in% c("IgA-HSP-P", "IgAL-P", "IER-P", "IGLL5-P")] <- "IgA-P"
levels[levels %in% c("Naive-B", "GC-B", "Mem.-B")] <- "Bcell"
seu$celltype_cr <- seu$celltype
levels(seu$celltype_cr) <- levels
Idents(seu) <- "celltype_cr"
DimPlot(seu, label = TRUE, repel = TRUE, cols = pal[17:20]) + NoAxes()
